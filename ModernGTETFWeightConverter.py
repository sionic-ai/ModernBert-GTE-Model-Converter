...
class ModernGTEWeightConverter:
    ...
    def _init_attention_weights(self, layer, prefix):
        ...
        if prefix != "layers.0.":
            ...
            # Layer normalization
            layer.attention.attnNorm.set_weights([
                attn_lm_weight
            ])
    ...
    def _init_ffn_weights(self, layer, prefix):
        ...
        # Intermediate dense
        layer.Wi.set_weights([
            self.pt_state_dict[f'{prefix}mlp.Wi.weight'].numpy().T
        ])

        # Output dense
        layer.Wo.set_weights([
            self.pt_state_dict[f'{prefix}mlp.Wo.weight'].numpy().T
        ])
    ...
    def _init_pooler_weights(self, tf_model):
        ...
        # 가중치 설정
        tf_model.final_norm.set_weights([
            final_norm_weight
        ])
